...
f = '/home/manhhv87/AICode/TMSCNet_Lettuce/data/test/'
...
name_debth = [p.split('/')[-1] for p in glob.glob(f+'Depth_*.png')]
...
y_h12 = np.argmax(y_h12, axis=1)        # Moi lam

y_label = model3.predict(rgb_ds)
y_label = np.argmax(y_label, axis=1)    # Take the index with the highest probability as the category
...
print("r2_fw = %f,   nrmse_fw = %f  " % (r2_fw,   nrmse_fw))
print("r2_dw = %f,   nrmse_dw = %f  " % (r2_dw,   nrmse_dw))
print("r2_h = %f,    nrmse_h = %f   " % (r2_h,    nrmse_h))
print("r2_dia = %f,  nrmse_dia = %f " % (r2_dia,  nrmse_dia))
print("r2_area = %f, nrmse_area = %f" % (r2_area, nrmse_area))
